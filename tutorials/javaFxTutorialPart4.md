...
As part of the effort to separate the code handling Duke's logic and duke.command.UI, let's _refactor_ the duke.command.UI-related code to its own class.
We call these duke.command.UI classes _controllers_. 
...
Without the annotation, we will have to make everything `public` and expose our duke.command.UI to unwanted changes.
...
Let's create a new `Main` class as the bridge between the existing logic in `Duke` and the duke.command.UI in `MainWindow`.
